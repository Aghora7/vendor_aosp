...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop com.lessaosp.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop com.lessaosp.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.lessaosp.com.username`
...
function pixelrebase() {
    ...
    git fetch "http://gerrit.lessaosp.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop com.lessaosp.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
