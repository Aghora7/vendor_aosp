...
def fetch_query_via_ssh(remote_url, query):
    ...
    out = subprocess.check_output(['ssh', '-x', '-p{0}'.format(
        port), userhost, 'gerrit', 'query', '--format=JSON --patch-sets --current-patch-set', query])
...
if __name__ == '__main__':
    ...
    default_gerrit = 'https://gerrit.lessaosp.com'
    ...
    parser.add_argument('-t', '--topic', metavar='',
                        help='pick all commits from a specified topic')
    parser.add_argument('-Q', '--query', metavar='',
    ...
    if args.topic:
        ...
        change_numbers = [str(r['number']) for r in sorted(
            reviews, key=cmp_to_key(cmp_reviews))]
    if args.query:
    ...
    if args.change_number:
        ...
        reviews = fetch_query(args.gerrit, ' OR '.join(
    ...
    for change in change_numbers:
        ...
        if patchset:
            try:
                mergables[-1]['fetch'] = [review['revisions'][x]['fetch']
                                          for x in review['revisions'] if review['revisions'][x]['_number'] == patchset][0]
                mergables[-1]['id'] = '{0}/{1}'.format(change, patchset)
                mergables[-1]['patchset'] = patchset
            except (IndexError, ValueError):
                args.quiet or print(
                    'ERROR: The patch set {0}/{1} could not be found, using CURRENT_REVISION instead.'.format(change, patchset))
    ...
    for item in mergables:
        ...
        if (item['status'] != 'OPEN' and item['status'] != 'NEW' and item['status'] != 'DRAFT') and not args.query:
            if args.force:
                ...
                print('Change status is ' +
                      item['status'] + '. Skipping the cherry pick.\nUse -f to force this pick.')
                continue
        ...
        if item['project'] in project_name_to_data and item['branch'] in project_name_to_data[item['project']]:
            ...
            print('WARNING: Project {0} has a different branch ("{1}" != "{2}")'.format(
        ...
        branch_commits_count = int(subprocess.check_output(
            ['git', 'rev-list', '--count', 'HEAD'], cwd=project_path))
        ...
        for i in range(0, check_picked_count):
            ...
            if 'Change-Id:' in output:
                ...
                if head_change_id.strip() == item['change_id']:
                    print(
                        'Skipping {0} - already picked in {1} as HEAD~{2}'.format(item['id'], project_path, i))
                    found_change = True
                    break
        if found_change:
            continue
        ...
        if not args.quiet:
            print(
                '--> Subject:       "{0}"'.format(item['subject'].encode('utf-8')))
            print('--> Project path:  {0}'.format(project_path))
            print(
                '--> Change number: {0} (Patch Set {1})'.format(item['id'], item['patchset']))
        ...
        if args.pull:
            cmd = ['git pull --no-edit', item['fetch']
                   [method]['url'], item['fetch'][method]['ref']]
        else:
            cmd = ['git fetch', item['fetch'][method]
                   ['url'], item['fetch'][method]['ref']]
        if args.quiet:
            cmd.append('--quiet')
        else:
            print(cmd)
            ...
            if result != 0:
                ...
                if result == 0:
                    ...
                    subprocess.call(cmd, cwd=project_path,
                                    shell=True, stdout=cmd_out, stderr=cmd_out)
                elif args.reset:
